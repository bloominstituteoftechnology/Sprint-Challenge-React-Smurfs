...
The diffrenece between client-side and server-side routing is how data is requested. In server-side, the request for data is made to the server, and the entire page is rendered for each request, this results in an overall slower experience although only the information requested is rendered. In client-side, although the initial load is longer because all info is requested initially, you can have components that render individually on the local host and render and update only certain parts when requested instead of having the entire page load

1.  What does HTTP stand for?
    http means hypertext transfer protocol, request and response protocol for the web browser
1.  What does CRUD stand for?
    create, read, update, delete. life cycle for our functionality in our APIs mechanisms are built in our api to allow this to happen
1.  Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
    put,post ,get ,update, delete
